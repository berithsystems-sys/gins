          ...
          <tbody>
            {vouchers.map((v, i) => (
              <tr key={i} style={{ ...ds.tr, background: selIdx === i ? YELLOW : 'transparent' }}>
                <td style={ds.td}>{fmtDate(v.date)}</td><td style={ds.td}>{v.narration || v.type}</td>
...
function LedgerMonthlySummary({ ledger, branchId, onBack, onDrill }: any) {
  ...
  useEffect(() => {
    const handleKeys = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') { 
        e.preventDefault(); 
        e.stopPropagation(); // STOP event from reaching Trial Balance
        onBack(); 
      }
    };
            ...
            {data.map((row, i) => {
              ...
              return (
                <tr key={i} style={{ ...ds.tr, background: selIdx === i ? YELLOW : 'transparent' }} onClick={() => onDrill(row.monthIdx)}>
                  <td style={ds.td}>{row.month}</td><td style={{...ds.td, textAlign:'right'}}>{fmtAmt(row.dr)}</td>
...
export default function TrialBalanceScreen({ branchId }: { branchId?: string }) {
  ...
  const calcBalance = useCallback((ledgerId: string) => {
    ...
    vouchers.forEach((v: any) => {
      ...
      entries.forEach((e: any) => { if (e.ledgerId === ledgerId) bal += (e.type === 'Dr' || e.entry_type === 'Dr' ? (e.amount || e.entry_amount) : -(e.amount || e.entry_amount)); });
    });
    return bal;
  }, [ledgers, vouchers]);
  ...
  const flatList = useMemo(() => {
    ...
    sortedGroups.forEach(grp => {
      ...
      if (expandedGroups.has(grp)) groupsData[grp].ledgers.forEach(l => list.push({ key: `L:${l.id}`, type: 'ledger', ledger: l }));
    });
    return list;
  ...
  useEffect(() => {
    // CRITICAL: If we are not in trial view, do not listen to keys here
            ...
            {sortedGroups.map(grp => (
              <React.Fragment key={grp}>
                <tr style={{ ...s.groupRow, background: selectedKey === `G:${grp}` ? YELLOW : 'transparent' }} onClick={() => setSelectedKey(`G:${grp}`)}>
                  ...
                  <td style={s.colDebit}>{fmtAmt(groupsData[grp].dr)}</td><td style={s.colCredit}>{fmtAmt(groupsData[grp].cr)}</td>
                </tr>
                {expandedGroups.has(grp) && groupsData[grp].ledgers.map(l => {
                  ...
                  return (
                    <tr key={l.id} style={{ ...s.ledgerRow, background: selectedKey === `L:${l.id}` ? YELLOW : 'transparent' }} onClick={() => setSelectedKey(`L:${l.id}`)}>
                      ...
                      <td style={s.colDebit}>{b >= 0 ? fmtAmt(b) : ''}</td><td style={s.colCredit}>{b < 0 ? fmtAmt(b) : ''}</td>
                    </tr>
                  );
                })}
              </React.Fragment>
