  ...
  const renderSection = (title: string, groupNames: string[]) => {
    ...
    return (
      <div className="flex flex-col h-full">
        ...
        <div className="flex justify-between text-sm font-black border-t-4 border-double border-red-700 pt-2 mt-4 px-1 text-red-700 bg-red-50/50">
          <span>TOTAL</span>
  ...
  return (
    <div id="pl-report" className="p-4 space-y-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="border border-red-700 bg-white shadow-xl p-6">
          <h3 className="text-sm font-black text-red-700 uppercase mb-6 border-b-2 border-red-700 pb-1 italic">Expenditure</h3>
          {renderSection('Expenses', ['Purchase Account', 'Direct Expenses', 'Indirect Expenses'])}
        </div>
        <div className="border border-red-700 bg-white shadow-xl p-6">
          <h3 className="text-sm font-black text-green-700 uppercase mb-6 border-b-2 border-green-700 pb-1 italic">Incomes</h3>
          {renderSection('Income', ['Sales Account', 'Direct Income', 'Indirect Income'])}
        </div>
      </div>

      <div className="bg-gray-100 p-4 border flex justify-between items-center text-sm font-black uppercase text-tally-teal">
         <span>Net Profit for the Period</span>
         <span className="text-xl">₹ {(getGroupTotal('Income') - getGroupTotal('Expenses')).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
      </div>
    </div>
  );
}
