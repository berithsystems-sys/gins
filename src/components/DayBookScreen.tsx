...
export default function DayBookScreen({ branchId, initialDate, fromDate: propFrom, toDate: propTo, user, onBackToGateway, onPrint }: any) {
  ...
  const handlePrintVoucher = (v: any) => {
    const fmtA = (n: number) => Math.abs(n).toLocaleString('en-IN', { minimumFractionDigits: 2 });
      ...
      <div style={s.reportHeader}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            ...
            <button onClick={() => selectedVoucher ? handlePrintVoucher(selectedVoucher) : handlePrintDayBook()}
              style={{ background: '#1f4e79', color: '#fff', border: 'none', borderRadius: 2, padding: '3px 10px', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: FONT }}>
              {selectedVoucher ? `Print Voucher (Alt+P)` : `Print Day Book (Alt+P)`}
            </button>
          </div>
        </div>
      </div>
